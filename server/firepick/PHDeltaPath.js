...
(typeof describe === 'function') && describe("firepick.PHDeltaPath", function() {
	...
	it("should have default options", function() {
		...
		var phdOptions = new PHDeltaPath(xyz,{
			e:131.636, 	// OPTION: effector equilateral triangle side
        	f:190.526,	// OPTION: base equilateral triangle side
			re:270, 	// OPTION: effector arm length
			rf:90, 		// OPTION: base arm length
	...
	it("thetaIterate(tau,prev) should traverse path angles", function() {
		...
		shouldPropertiesEqualT(angles, {tau:0, theta1:30.082, theta2:30.082, theta3:30.082});
		var xyz = phd.delta.calcXYZ(angles);
		shouldPropertiesEqualT(xyz, {x:0, y:0, z:-50});
		angles = phd.thetaIterate(0.1, angles);
		shouldPropertiesEqualT(angles, {tau:0.1, theta1:30.082, theta2:30.034, theta3:30.131});
		xyz = phd.delta.calcXYZ(angles);
		shouldPropertiesEqualT(xyz, {x:0.245, y:-0.001, z:-49.999});
		angles = phd.thetaIterate(0.2, angles);
		shouldPropertiesEqualT(angles, {tau:0.2, theta1:30.090, theta2:29.501, theta3:30.680});
		xyz = phd.delta.calcXYZ(angles);
		shouldPropertiesEqualT(xyz, {x: 3.002, y:-0.005, z:-49.998});
		angles = phd.thetaIterate(0.3, angles);
		shouldPropertiesEqualT(angles, {tau:0.3, theta1:30.242, theta2:27.953, theta3:32.491});
		xyz = phd.delta.calcXYZ(angles);
		shouldPropertiesEqualT(xyz, {x:11.553, y: 0.001, z:-50.001});
		angles = phd.thetaIterate(0.4, angles);
		shouldPropertiesEqualT(angles, {tau:0.4, theta1:30.984, theta2:25.493, theta3:36.244});
		xyz = phd.delta.calcXYZ(angles);
		shouldPropertiesEqualT(xyz, {x:27.413, y:-0.000, z:-50.000});
		angles = phd.thetaIterate(0.5, angles);
		shouldPropertiesEqualT(angles, {tau:0.5, theta1:32.874, theta2:23.116, theta3:41.885});
		xyz = phd.delta.calcXYZ(angles);
		shouldPropertiesEqualT(xyz, {x:48.016, y:-0.000, z:-50.000});
		angles = phd.thetaIterate(0.6, angles);
		shouldPropertiesEqualT(angles, {tau:0.6, theta1:35.603, theta2:21.767, theta3:47.875});
		xyz = phd.delta.calcXYZ(angles);
		shouldPropertiesEqualT(xyz, {x:67.076, y: 0.000, z:-50.000});
		angles = phd.thetaIterate(0.7, angles);
		shouldPropertiesEqualT(angles, {tau:0.7, theta1:38.140, theta2:21.325, theta3:52.551});
		xyz = phd.delta.calcXYZ(angles);
		shouldPropertiesEqualT(xyz, {x:80.496, y: 0.000, z:-49.999});
		angles = phd.thetaIterate(0.8, angles);
		shouldPropertiesEqualT(angles, {tau:0.8, theta1:39.694, theta2:21.265, theta3:55.167});
		xyz = phd.delta.calcXYZ(angles);
		shouldPropertiesEqualT(xyz, {x:87.547, y:0.002, z:-49.996});
		angles = phd.thetaIterate(0.9, angles);
		shouldPropertiesEqualT(angles, {tau:0.9, theta1:40.227, theta2:21.275, theta3:56.029});
		xyz = phd.delta.calcXYZ(angles);
		shouldPropertiesEqualT(xyz, {x:89.800, y:-0.000, z:-50.000});
		angles = phd.thetaIterate(1, angles);
		shouldPropertiesEqualT(angles, {tau:1, theta1:40.276, theta2:21.277, theta3:56.107});
		xyz = phd.delta.calcXYZ(angles);
	...
	it("thetaIterate(tau,prev) should traverse path angles", function() {
		...
		shouldPropertiesEqualT(angles, {tau:0.1, theta1:30.082, theta2:30.034, theta3:30.131});
		xyz = phd.delta.calcXYZ(angles);
		shouldPropertiesEqualT(xyz, {x:0.246, y:-0.001, z:-49.999});
		angles = phd.thetaIterate(0.2, angles);
		shouldPropertiesEqualT(angles, {tau:0.2, theta1:30.089, theta2:29.5, theta3:30.680});
		xyz = phd.delta.calcXYZ(angles);
		shouldPropertiesEqualT(xyz, {x: 3.002, y:-0.010, z:-49.996});
		angles = phd.thetaIterate(0.3, angles);
		shouldPropertiesEqualT(angles, {tau:0.3, theta1:30.240, theta2:27.953, theta3:32.489});
		xyz = phd.delta.calcXYZ(angles);
		shouldPropertiesEqualT(xyz, {x:11.548, y: -0.005, z:-49.998});
		angles = phd.thetaIterate(0.4, angles);
		shouldPropertiesEqualT(angles, {tau:0.4, theta1:30.985, theta2:25.493, theta3:36.245});
		xyz = phd.delta.calcXYZ(angles);
		shouldPropertiesEqualT(xyz, {x:27.414, y:-0.000, z:-50.000});
		angles = phd.thetaIterate(0.5, angles);
		shouldPropertiesEqualT(angles, {tau:0.5, theta1:32.874, theta2:23.116, theta3:41.885});
		xyz = phd.delta.calcXYZ(angles);
		shouldPropertiesEqualT(xyz, {x:48.016, y:-0.000, z:-50.000});
		angles = phd.thetaIterate(0.6, angles);
		shouldPropertiesEqualT(angles, {tau:0.6, theta1:35.604, theta2:21.769, theta3:47.875});
		xyz = phd.delta.calcXYZ(angles);
		shouldPropertiesEqualT(xyz, {x:67.073, y: 0.000, z:-50.002});
		angles = phd.thetaIterate(0.7, angles);
		shouldPropertiesEqualT(angles, {tau:0.7, theta1:38.140, theta2:21.322, theta3:52.550});
		xyz = phd.delta.calcXYZ(angles);
		shouldPropertiesEqualT(xyz, {x:80.502, y: 0.005, z:-49.995});
		angles = phd.thetaIterate(0.8, angles);
		shouldPropertiesEqualT(angles, {tau:0.8, theta1:39.694, theta2:21.264, theta3:55.166});
		xyz = phd.delta.calcXYZ(angles);
		shouldPropertiesEqualT(xyz, {x:87.547, y:0.006, z:-49.995});
		angles = phd.thetaIterate(0.9, angles);
		shouldPropertiesEqualT(angles, {tau:0.9, theta1:40.227, theta2:21.275, theta3:56.029});
		xyz = phd.delta.calcXYZ(angles);
		shouldPropertiesEqualT(xyz, {x:89.800, y:-0.000, z:-49.999});
		angles = phd.thetaIterate(1, angles);
		shouldPropertiesEqualT(angles, {tau:1, theta1:40.275, theta2:21.277, theta3:56.107});
		xyz = phd.delta.calcXYZ(angles);
		shouldPropertiesEqualT(xyz, {x:90, y:0, z:-50});
	});
})
