var firepick; (function (firepick) {
  var SpiralIterator = (function () {
    ...
    SpiralIterator.prototype.next = function() {
      if (this.x > this.xMax || this.y > this.yMax) { return false; }
      this.x += this.dx; 
      this.y += this.dy; 
      switch (this.state) {
	case 0: this.state = 1; this.mx++; this.my++; this.dx = 0; this.dy = 1; break;
	case 1: if (this.y >= this.my) { this.state = 2; this.dx = -1; this.dy = 0; } break;
	case 2: if (this.x <= -this.mx ) { this.state = 3; this.dx = 0; this.dy = -1; } break;
	case 3: if (this.y <= -this.my ) { this.state = 4; this.dx = 1; this.dy = 0; } break;
	case 4: if (this.mx <= this.x) { this.state = 5; this.dx = 0; this.dy = 1; } break;
	case 5: if (0 <= this.y) { this.state = 1; this.mx++, this.my++; this.x++; } break;
	case 6: 
	  if (this.dx != 0) {
	  ...
	  break;
	default: throw "unknown state"; break;
      }
